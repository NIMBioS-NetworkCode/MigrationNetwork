...
## USERS MUST CHANGE THIS CODE TO SET UP SPECIES-SPECIFIC FUNCTIONS
...
f_function <- function() {
  ...
  # Enter node update function here:
  
  survi <- alpha[[type]][[ind]]$s_node*N[[t]][[type]] # Constant node survival properties
  repro <- 0 # Zero node reproduction
  trans <- 0 # Zero transitions from other classes
...
p_function <- function(){
  ...
  ## Enter edge transition update function here:
  
  p_new <- beta[[type]][[ind]]$p_ij # Constant edge transition probabilities
...
s_function <- function(){
  ...
  ## Enter edge survival update function here:
  
  s_new <- beta[[type]][[ind]]$s_ij # Constant edge survival rates
